...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    /* base case is the current node is null*/
    if (cur == 0) {
    ...
    else if (compare(cur->val,val) == 1){   //current value is less than the node
        cur->left = _addNode(cur->left, val); //recursive call
    }
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (currentNode != NULL){
        if (compare(currentNode->val, val) == 0)
            return 1;
        else if (compare(currentNode->val, val) == 1)
...
struct Node *_removeLeftMost(struct Node *cur)
{
    /*write this*/
    assert(cur!=NULL); 
    /* base case current nodes left child is null*/
    if (cur->left == NULL){
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    else if (compare(cur->val,val) == 1){   //value is less than current node
