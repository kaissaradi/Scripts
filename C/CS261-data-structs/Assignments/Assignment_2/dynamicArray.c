...
{	
	...
	/* Allocate memory for a new array of size newCap and copy data into new array */
	assert(v != 0);
	...
	/* Delete the old underlying array and set new values for v*/
    free(v->data);
	v->data = newData;
	v->capacity = newCap;
}
...
TYPE getDynArr(DynArr *v, int pos)
{
	/* FIXME: You will write this function */
	assert(v != 0);
	assert(v->size > 0 && pos >= 0 && pos < v->size);
...
void swapDynArr(DynArr *v, int i, int  j)
{
	/* FIXME: You will write this function */
	assert(v != 0);
	assert(v->size > 0 && i >= 0 && i < v->size && j >= 0 && j < v->size);
	TYPE temp = v->data[i];
	v->data[i] = v->data[j];
	v->data[j] = temp;
}
...
void removeAtDynArr(DynArr *v, int idx)
{
	/* FIXME: You will write this function */
	assert(v != 0);
	assert(v->size > 0 && idx >= 0 && idx < v->size);
