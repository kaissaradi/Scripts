...
void linkedListPrint(struct LinkedList* deque)
{
	/* FIXME: You will write this function */
	assert(deque != 0);
	if(linkedListIsEmpty == 1){
		printf("\nThe List is empty!\n");
	}
	else{
		struct Link *currentLnk = deque->frontSentinel;
		printf("\nLinked List Values:	");
		do{
			currentLnk = currentLnk->next;
			printf("%i ", currentLnk->value);
		}while(currentLnk->next != deque->backSentinel);
		printf("\n");
	}
}
...
int linkedListContains(struct LinkedList* bag, TYPE value)
{
	...
	int contains = 0;
	struct Link* currentLnk = bag->frontSentinel->next;
	while (currentLnk != bag->backSentinel && contains == 0) {
		if (currentLnk->value == value){
			contains = 1;
		}
		currentLnk = currentLnk->next;
	}
...
void linkedListRemove(struct LinkedList* bag, TYPE value)
{
	...
	assert(!linkedListIsEmpty(bag));
	struct Link* currentLnk = bag->frontSentinel->next;
	while (currentLnk != bag->backSentinel) {
		if (currentLnk->value == value){
			removeLink ( bag, currentLnk);
			return;
		}
		currentLnk = currentLnk->next;
	}
